...
args = get_arguments()
os.makedirs(args.save_gb_model_path, exist_ok=True)
...
def trainer(gpu, args, device):
    ...
    bst_all, y_val_pred_all = get_predictions(
    param = trials.argmin,
    dM_train = dtrain,
    dM_val = dvalid
)
    ...
    logging.info("Validation set:")
    bst_all_cls, y_val_pred_all_cls = get_predictions(
    param = trials.argmin,
    dM_train = dtrain_all_cls,
    dM_val = dvalid_all_cls
)
    ...
    logging.info("Validation set:")
    bst_cls, y_val_pred_cls = get_predictions(
    param = trials.argmin,
    dM_train = dtrain_cls,
    dM_val = dvalid_cls
)
    ...
    bst_all_test.save_model(
        os.path.join(args.save_gb_model_path, "gb_all.json")
    )

    bst_all_cls_test.save_model(
        os.path.join(args.save_gb_model_path, "gb_all_cls.json")
    )

    bst_cls_test.save_model(
        os.path.join(args.save_gb_model_path, "gb_cls.json")
    )
    ...
    with open(
        os.path.join(args.save_gb_model_path, "ensemble_weights.json"),
        "w"
    ) as f:
